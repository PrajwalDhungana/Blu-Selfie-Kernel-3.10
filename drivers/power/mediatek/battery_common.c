...
//LC humingming 20150528 add for runin BAT_OVER_SOC_RECHARGE 	115
...
#endif	
//LC humingming 20150528 add end 
...
//LC humingming 20150528 add for runin BAT_OVER_SOC_RECHARGE
#if defined(BAT_OVER_SOC_RECHARGE)
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	if ((BMT_status.UI_SOC == 100) && (BMT_status.charger_exist == KAL_TRUE)&&(BMT_status.bat_vol>=4350))//lc mike_zhu 20150801
...
static kal_bool mt_battery_100Percent_tracking_check(void)
{
	...
	if ((BMT_status.bat_full == KAL_TRUE)&&(BMT_status.bat_vol>=4350))	/* charging full first, UI tracking to 100% */
	{
...
/*humingming add for fan5405 charging full vol low end*/

static kal_uint8 BatAvgTempSize = 30;	//BATTERY_AVERAGE_TEMPERATURE_SIZE;
static kal_uint8 FirstTime = 1;
static kal_uint8 BatTempIndex = 0;
static kal_int32 mt_battery_average_temprature(kal_uint32 *bufferdata, kal_int32 data)
{
	kal_uint32 i = 0;
	kal_int32 avgdata = 0,temperature_sum = 0;

	if((1 == FirstTime)||(BatAvgTempSize >5))//first time
	{
		FirstTime = 0;
		for(i = 0;i < BatAvgTempSize;i++)
		{
			bufferdata[i] = data;
		}
		temperature_sum = data * BatAvgTempSize;
	}
	else
	{
		bufferdata[BatTempIndex] = data;
		printk("hzy data = %d\n",data);
		for(i = 0;i < BatAvgTempSize;i++)
		{
			printk("hzy data[%d] = %d\n",i,bufferdata[i]);
			temperature_sum += bufferdata[i];
		}
	}

    avgdata = (temperature_sum)/BatAvgTempSize;
   if(avgdata >=56)
    {
          BatAvgTempSize =30;   
    }
	...
	printk("hzy avgdata = %d,BatTempIndex = %d \n",avgdata,BatTempIndex);
	BatTempIndex++;
    if (BatTempIndex >= BatAvgTempSize)
        BatTempIndex = 0;

	battery_xlog_printk(BAT_LOG_FULL, "bufferdata[%d]= (%d) \n", BatTempIndex,bufferdata[BatTempIndex]);
	return avgdata;
}
/*hzy ad for battery avg temprature new method end*/
					    ...
					    kal_int32 *sum, kal_uint8 batteryIndex)
{
	...
	*sum += data;
	...
	battery_log(BAT_LOG_FULL, "battery_common.c  bufferdata[%d]= (%d)\n", batteryIndex,
...
//	BMT_status.temperature =   mt_battery_average_method(BATTERY_AVG_TEMP, &batteryTempBuffer[0], temperature, &temperature_sum,   batteryIndex);
  	BMT_status.temperature = mt_battery_average_temprature(&batteryTempBuffer[0],temperature) -2;///mike_zhu 
	}
...
void do_chrdet_int_task(void)
{
	...
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if (BMT_status.bat_vol > 0) {
			mt_battery_update_status();
			mt_battery_GetBatteryData();   //mtk  add			
			printk("lj: BMT_status.charger_vol first :%d try_times=0\n", BMT_status.charger_vol );
			if (BMT_status.charger_vol < 4000)
			{
				for(try_times = 1; (try_times < 10) && (get_charger_detect_status()) ; try_times++)
				{
					msleep(500);
					mt_battery_GetBatteryData();   //mtk  add
					printk("lj: BMT_status.charger_vol :%d try_times:%d\n", BMT_status.charger_vol, try_times );
					if (BMT_status.charger_vol > 4000)
					{
						printk("lj: BMT_status.charger_vol :%d try_times:%d >4000, break out \n", BMT_status.charger_vol, try_times );
						mt_battery_update_status();
						break ;
					}
				}
			...
			battery_log(BAT_LOG_CRTI, "[do_chrdet_int_task][HHL]skip_battery_update = ture .\n"); //mtk  add
     			 //  msleep(2000);  //mtk  add
