...
void power_on_self(void)
{
	...
	tpd->reg=regulator_get(tpd->tpd_dev,MT6325_POWER_LDO_VGP2); // get pointer to regulator structure
		if (IS_ERR(tpd->reg)) {
		printk("lj tp  regulator_get() failed  =========!\n");
	}
	printk("lj tp  regulator_get() ok  =========!\n");

	ret=regulator_set_voltage(tpd->reg, 2800000, 2800000);  // set 2.8v
	if (ret)
		printk("lj regulator_set_voltage() failed!\n");
	ret=regulator_enable(tpd->reg);  //enable regulator
	if (ret)
		printk("lj regulator_enable() failed!\n");
}
#endif
#else
...
void power_on_0(void)
{
	...
	ret=regulator_set_voltage(tpd->reg, 0, 0);  // set 2.8v
	if (ret)
		printk("lj regulator_set_voltage() failed!\n");
...
{
   printk("msg2238 tpd_probe TPD probe\n");   
    
    if (client == NULL)
    {
        printk("msg2238  tpd_probe i2c client is NULL\n");
        return -1;
    }
    g_I2cClient = client;
...
{  
    printk("msg2238 TPD init device driver  \n");
power_on_self_1();
/*
...
*/
    ...
    {
        printk("msg2238 unable to add i2c driver.\n");
    ...
    {
        printk("msg2238 add error touch panel driver.\n");
...
{
    printk("msg2238 touch panel driver init I2C_BUS_ID=%d\n",I2C_BUS_ID);
    i2c_register_board_info(I2C_BUS_ID, &i2c_tpd, 1);
    if (tpd_driver_add(&tpd_device_driver) < 0)
    {
       printk("msg2238 TPD add driver failed\n");
    }
...
{
    printk("msg2238 touch panel driver exit\n");
