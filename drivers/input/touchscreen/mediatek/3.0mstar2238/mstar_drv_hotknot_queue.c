...
int PushQueue(u8 * pBuf, u16 nLength)
{
    ...
    printk("msg2238  *** Before PushQueue: _gQFront = %d, _gQRear = %d ***\n", _gQFront, _gQRear);

    if(_gQRear >= _gQFront)
    {
        if(nPushLen > 0 && _gQFront == 0 && _gQRear == _gQSize-1)    //full
        {
            printk("msg2238  *** PushQueue: RESULT_OVERPUSH ***\n");
    ...
    printk("msg2238  *** After PushQueue: _gQFront = %d, _gQRear = %d ***\n", _gQFront, _gQRear); 
    return nPushLen;     
}
...
int PopQueue(u8 * pBuf, u16 nLength)
{
    ...
    printk("msg2238  *** Before PopQueue: _gQFront = %d, _gQRear = %d ***\n", _gQFront, _gQRear);  

    if(_gQRear >= _gQFront)
    {
        if(nPopLen > 0 && _gQRear == _gQFront)    //empty
        {
            printk("msg2238  *** PushQueue: RESULT_OVERPOP ***\n");        
    ...
    printk("msg2238  *** After PopQueue: _gQFront = %d, _gQRear = %d ***\n", _gQFront, _gQRear);   
    return nPopLen;    
}
