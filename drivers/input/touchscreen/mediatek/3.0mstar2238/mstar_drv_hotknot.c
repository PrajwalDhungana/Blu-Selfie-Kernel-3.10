...
void _HotKnotRcvInterruptHandler(u8 *pPacket, u16 nLength)    //receive hotknot packet in interrupt 
{
    ...
    { 
        ...
        if(nPacketDataLen >= nQPushLen)
        {
            ...
            {
                printk("msg2238  Error: HotKnot# Over push data into queue.");
            }
#ifdef CONFIG_ENABLE_HOTKNOT_RCV_BLOCKING
            else
            {
                ...
                {
                    _gHKRcvFlag = 1;    
                    printk("msg2238  *** wait up receive_wait. ***\n");                                        
        ...
        {
            printk("msg2238  Error: HotKnot# Receive data error.");
        }        
...
{    
    ...
    //{
    //    printk("msg2238  02X ", pCmd->pSndData[i]); 
    //}
    ...
    //{
    //    printk("msg2238  02X ", pCmd->pRcvData[i]); 
    //}    
...
void _DrvHandleHotKnotRcv(DrvCmd_t *pCmd)
{
    ...
    {
        printk("msg2238  ShowHotKnotData: No hotknot data in first check.\n");
    }
    else
    {
    ...
    {
        printk("msg2238  *** receive_get_data ***\n");     
        int nDataLen = -1;    
        if(ShowHotKnotData(&nQueueData, 1) <= 0)    //check how many bytes to fetch
        {
            printk("msg2238  ShowHotKnotData: No hotknot data\n");
        }
        else
        {
            nDataLen = PopHotKnotData(pRcv->szData, (nQueueData&0x7F) + 1);
