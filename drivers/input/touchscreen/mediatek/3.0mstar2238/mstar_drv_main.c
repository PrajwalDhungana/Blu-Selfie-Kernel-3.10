...
/*=============================================================*/
///lc mike_zhu 20150521 0xC4 to B2
...
{    
    ...
    if (pBuffer != NULL)
    {
        ...
        if (pValid)
        {
            ...
            printk("msg2238  pTmpFilePath = %s\n", pTmpFilePath);

            strcat(szFilePath, pTmpFilePath);
            strcat(szFilePath, ".bin");

            printk("msg2238 szFilePath = %s\n", szFilePath);
        ...
        {
            printk("msg2238 The file type of the update firmware bin file is not a .bin file.\n");
        }
    }
    else
    {
        printk("msg2238 The file path of the update firmware bin file is NULL.\n");
    }
...
ssize_t DrvMainProcfsFirmwareDebugRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    for (i = 0; i < _gDebugRegCount; i ++)
    {
        ...
        printk("msg2238 reg(0x%02X,0x%02X)=0x%04X\n", nBank, nAddr, szRegData[i]);
...
{    
    ...
    if (pBuffer != NULL)
    {
        printk("msg2238 *** pBuffer[0] = %c ***\n", pBuffer[0]);
        printk("msg2238 *** pBuffer[1] = %c ***\n", pBuffer[1]);
        printk("msg2238 *** pBuffer[2] = %c ***\n", pBuffer[2]);
        printk("msg2238 *** pBuffer[3] = %c ***\n", pBuffer[3]);
        printk("msg2238 *** pBuffer[4] = %c ***\n", pBuffer[4]);
        printk("msg2238 *** pBuffer[5] = %c ***\n", pBuffer[5]);

        printk("msg2238 nCount = %d\n", (int)nCount);
        ...
        {
            printk("msg2238 copy_from_user() failed\n");
        ...
        {
            printk("msg2238 pCh = %s\n", pCh);
            
            _gDebugReg[i] = DrvCommonConvertCharToHexDigit(pCh, strlen(pCh));

            printk("msg2238 _gDebugReg[%d] = 0x%04X\n", i, _gDebugReg[i]);
            i ++;
        }
        _gDebugRegCount = i;
        
        printk("msg2238 _gDebugRegCount = %d\n", _gDebugRegCount);
    ...
    if (pBuffer != NULL)
    {
        ...
        while ((pCh = strsep((char **)&pData, " ,")) && (i < 2))
        {
            ...
            {	
                ...
                printk("msg2238 _gDebugRegValue[%d] = 0x%04X\n", k, _gDebugRegValue[k]);
                k ++;
            }
        ...
        for (i = 0; i < _gDebugRegCount; i ++)
        {
            ...
            printk("msg2238 _gDebugReg[%d] = 0x%04X, _gDebugRegValue[%d] = 0x%04X\n", i, _gDebugReg[i], i , _gDebugRegValue[i]); // add for debug
        }
...
ssize_t DrvMainProcfsMpTestRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    printk("msg2238 *** ctp mp test status = %d ***\n", DrvIcFwLyrGetMpTestResult());
...
{    
    ...
    if (pBuffer != NULL)
    {
        ...
        printk("msg2238 Mp Test Mode = 0x%x\n", nMode);
        ...
        {
            printk("msg2238 *** Undefined MP Test Mode ***\n");
        }
    }
...
{    
    ...
    {
        sscanf(pBuffer, "%x", &nMode);   
        printk("msg2238 firmware mode = 0x%x\n", nMode);
        ...
        {
            printk("msg2238 *** Undefined Firmware Mode ***\n");
        }

        DrvPlatformLyrEnableFingerTouchReport(); 
    }

    printk("msg2238 *** g_FirmwareMode = 0x%x ***\n", g_FirmwareMode);
...
ssize_t DrvMainProcfsFirmwareSensorRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    {
        printk("msg2238 Undefined debug mode packet format : 0x%x\n", g_FirmwareInfo.nLogModePacketHeader);
        nLength = 0;
    }
#elif defined(CONFIG_ENABLE_TOUCH_DRIVER_FOR_SELF_IC)
...
ssize_t DrvMainKObjectPacketShow(struct kobject *pKObj, struct kobj_attribute *pAttr, char *pBuf)
{
    ...
    if (strcmp(pAttr->attr.name, "packet") == 0)
    {
        ...
        {
            printk("msg2238 g_FirmwareMode=%x, g_LogModePacket[0]=%x, g_LogModePacket[1]=%x\n", g_FirmwareMode, g_LogModePacket[0], g_LogModePacket[1]);
            printk("msg2238 g_LogModePacket[2]=%x, g_LogModePacket[3]=%x\n", g_LogModePacket[2], g_LogModePacket[3]);
            printk("msg2238 g_LogModePacket[4]=%x, g_LogModePacket[5]=%x\n", g_LogModePacket[4], g_LogModePacket[5]);
...
#endif
            {
                ...
                printk("msg2238 nLength = %d\n", nLength);
            }
            else
            {
                printk("msg2238 CURRENT MODE IS NOT DEBUG MODE/WRONG DEBUG MODE HEADER\n");
//            nLength = 0;
            }
        }
        else
        {
            printk("msg2238 g_LogModePacket is NULL\n");
//            nLength = 0;
        }
    }
    else
    {
        printk("msg2238 pAttr->attr.name = %s \n", pAttr->attr.name);
//        nLength = 0;
    }
...
#else
    printk("msg2238 g_GestureWakeupMode = 0x%x\n", g_GestureWakeupMode[0]);
...
{    
    ...
    if (pBuffer != NULL)
    {
        ...
        while ((pCh = strsep((char **)&pBuffer, " ,")) && (i < 2))
        {
            ...
            printk("msg2238 nWakeupMode[%d] = 0x%04X\n", i, nWakeupMode[i]);
            i ++;
        }
#else
        ...
        printk("msg2238 g_GestureWakeupMode = 0x%x,  0x%x\n", g_GestureWakeupMode[0], g_GestureWakeupMode[1]);
    }
...
{    
    ...
    if (pBuffer != NULL)
    {
        ...
        while ((pCh = strsep((char **)&pBuffer, " ,")) && (i < 2))
        {
            ...
            printk("msg2238 ucGestureMode[%d] = 0x%04X\n", i, ucGestureMode[i]);
            i ++;
        }
...
//            input_report_key(g_InputDevice, RESERVER42, 0);
        ...
        {
            printk("msg2238 *** Undefined Gesture Debug Mode ***\n");
        }
    }
...
ssize_t DrvMainKObjectGestureDebugShow(struct kobject *pKObj, struct kobj_attribute *pAttr, char *pBuf)
{
    ...
    if (strcmp(pAttr->attr.name, "gesture_debug") == 0)
    {
        ...
        {
            printk("msg2238 g_LogGestureDebug[0]=%x, g_LogGestureDebug[1]=%x\n", g_LogGestureDebug[0], g_LogGestureDebug[1]);
            printk("msg2238 g_LogGestureDebug[2]=%x, g_LogGestureDebug[3]=%x\n", g_LogGestureDebug[2], g_LogGestureDebug[3]);
            printk("msg2238 g_LogGestureDebug[4]=%x, g_LogGestureDebug[5]=%x\n", g_LogGestureDebug[4], g_LogGestureDebug[5]);
            ...
            {
                printk("msg2238 CURRENT MODE IS NOT GESTURE DEBUG MODE/WRONG GESTURE DEBUG MODE HEADER\n");
//            nLength = 0;
            }
        }
        else
        {
            printk("msg2238 g_LogGestureDebug is NULL\n");
//            nLength = 0;
        }
    }
    else
...
ssize_t DrvMainProcfsGestureInforModeRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    {
        printk("msg2238 *** Undefined GESTURE INFORMATION MODE ***\n");
    }
...
{    
    ...
    printk("msg2238 *** _gLogGestureInforType type = 0x%x ***\n", _gLogGestureInforType);
...
ssize_t DrvMainProcfsReportRateRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    printk("msg2238 g_InterruptCount = %d, g_ValidTouchCount = %d\n", g_InterruptCount, g_ValidTouchCount);
    ...
    printk("msg2238 End time : %lu sec, %lu msec\n", tEndTime.tv_sec, tEndTime.tv_usec); 

    printk("msg2238 Elapsed time : %lu sec\n", nElapsedTime); 
    ...
    printk("msg2238 g_InterruptReportRate = %d, g_ValidTouchReportRate = %d\n", g_InterruptReportRate, g_ValidTouchReportRate);
...
ssize_t DrvMainProcfsGloveModeWrite(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    if (pBuffer != NULL)
    {
        ...
        printk("msg2238 glove mode = 0x%x\n", nGloveMode);
        ...
        {
            printk("msg2238 *** Undefined Glove Mode ***\n");
        }
...
s32 DrvMainTouchDeviceInitialize(void)
{
    ...
    printk("msg2238 DrvMainTouchDeviceInitialize -----g_ChipType=%d---\n",g_ChipType);
    ...
    if (g_ChipType != 0) // To make sure TP is attached on cell phone.
    {
        ...
        {
            printk("msg2238 call hwmsen_attach() failed = %d\n", nErr);
        }
#endif
#endif //CONFIG_ENABLE_PROXIMITY_DETECTION
    }
...
static s32 _DrvMainCreateProcfsDirEntry(void)
{
    ...
    {
       printk("msg2238 Failed to create procfs file node(%s)!\n", PROC_NODE_CHIP_TYPE);
    }   
    else 
    {
        printk("msg2238 Create procfs file node(%s) OK!\n", PROC_NODE_CHIP_TYPE);
    }
    ...
    if (!g_TouchKSet)
    {
        printk("msg2238 *** kset_create_and_add() failed, nRetVal = %d ***\n", nRetVal);
    ...
    if (!g_TouchKObj)
    {
        printk("msg2238 *** kobject_create() failed, nRetVal = %d ***\n", nRetVal);
    ...
    if (nRetVal != 0)
    {
        printk("msg2238 *** kobject_add() failed, nRetVal = %d ***\n", nRetVal);
    ...
    if (nRetVal != 0)
    {
        printk("msg2238 *** sysfs_create_file() failed, nRetVal = %d ***\n", nRetVal);
