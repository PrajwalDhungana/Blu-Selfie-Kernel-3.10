...
#if 1//def SLT_DEVINFO_CTP
extern unsigned char CFG_VER_buff[12];	//read the version name
#endif
/*=============================================================*/
...
#include "msg2xxx_zzzz_update_bin.h"
...
void _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId(void)
{
    ...
    printk("swf553updatefw  nCrcMainA=0x%x, nCrcInfoA=0x%x, nCrcMainB=0x%x, nCrcInfoB=0x%x\n", nCrcMainA, nCrcInfoA, nCrcMainB, nCrcInfoB);
    ...
    if (nCrcMainA == nCrcMainB && nCrcInfoA == nCrcInfoB) // Case 1. Main Block:OK, Info Block:OK
    {
        ...
        {
            nUpdateBinMajor = msg2xxx_zzzz_update_bin[0xBFF5]<<8 | msg2xxx_zzzz_update_bin[0xBFF4];
            nUpdateBinMinor = msg2xxx_zzzz_update_bin[0xBFF7]<<8 | msg2xxx_zzzz_update_bin[0xBFF6];
        }
        ...
        DrvFwCtrlGetCustomerFirmwareVersion(&nMajor, &nMinor, &pVersion); //fang

        printk("swf553updatefw eSwId=0x%x, nMajor=%d, nMinor=%d, nUpdateBinMajor=%d, nUpdateBinMinor=%d\n", eSwId, nMajor, nMinor, nUpdateBinMajor, nUpdateBinMinor);
            ...
            if (eSwId < MSG22XX_SW_ID_UNDEFINED && eSwId != 0x0000 && eSwId != 0xFFFF)
            {
                ...
                else if (eSwId == MSG22XX_SW_ID_ZZZZ)
                {
                    for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE+1); i ++)
                    {
                        if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) // i < 48
                        {
                            _DrvFwCtrlStoreFirmwareData(&(msg2xxx_zzzz_update_bin[i*1024]), 1024);
                        }
                        else // i == 48
                        {
                            _DrvFwCtrlStoreFirmwareData(&(msg2xxx_zzzz_update_bin[i*1024]), 512);
                        }
                    }
                }
...
u8 DrvFwCtrlGetChipType(void)
{
    ...
    {
        nChipType = 0;   //swf modify
    }

    printk("***swf55 self_fw_control  Chip Type = 0x%x ***\n", nChipType);
...
void DrvFwCtrlGetCustomerFirmwareVersion(u16 *pMajor, u16 *pMinor, u8 **ppVersion)
{
    ...
    { 
        ...
        mutex_lock(&g_Mutex);  //swf delete ,otherwise phone reset always
...
//        RegSet16BitValue(0x160C, RegGet16BitValue(0x160C) & (~0x01));      
	...
	#if 1///def SLT_DEVINFO_CTP
	memset(CFG_VER_buff,0,12);
     sprintf(CFG_VER_buff, "%03d%03d", *pMajor, *pMinor);
printk("msg2238  CFG_VER_buff=%s\n",CFG_VER_buff);	
#endif
...
void DrvFwCtrlHandleFingerTouch(void)
{
  ...
  // printk("swf55cc self_fw_control.c  interrupt tou XX  YY *** %s() ***\n", __func__);
...
//	printk("swf55cc self_fw_control.c  xy_report  packet=%d . \n",_DrvFwCtrlParsePacket(pPacket, nReportPacketLength, &tInfo));
...
printk("swf551 line=%d , g_ChipType=0x%02X  ..\n",__LINE__,g_ChipType);

    if (g_ChipType == CHIP_TYPE_MSG21XXA)     //0x01
    {
        _DrvFwCtrlMsg21xxaCheckFirmwareUpdateBySwId();
    }
    else if (g_ChipType == CHIP_TYPE_MSG22XX)     //0x7A 
    {
        _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId();  //fang  go to "msg2xxx_xxxx_update_bin" finally
    }
    else
    {    
        DBG("This chip type (%d) does not support update firmware by sw id\n", g_ChipType);
