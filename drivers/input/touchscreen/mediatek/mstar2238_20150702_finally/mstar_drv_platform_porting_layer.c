...
#include <cust_eint.h>///lc mike_zhu 20150521--add 
/*=============================================================*/
...
void DrvPlatformLyrTouchDeviceRegulatorPowerOn(void)
{
    ...
    mdelay(20);
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
    hwPowerOn(MT6325_POWER_LDO_VGP1, VOL_2800, "TP"); // For specific MTK BB chip(ex. MT6582), need to enable this function call for correctly power on Touch IC.
...
//    gpio_set_value(MS_TS_MSG_IC_GPIO_RST, 1); 
    ...
    gpio_set_value(MS_TS_MSG_IC_GPIO_RST, 1);
    mdelay(25); 
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)   //swf delett
...
s32 DrvPlatformLyrTouchDeviceRegisterFingerTouchInterruptHandler(void)
{
    ...
    {    	
        ...
        mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM,CUST_EINT_TOUCH_PANEL_POLARITY/* CUST_EINT_TOUCH_PANEL_TYPE*//* EINTF_TRIGGER_RISING */, _DrvPlatformLyrFingerTouchInterruptHandler, 1);

        mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
#endif

  //  printk("swf55 interrupt  register start11111  \n");
		///////////////////////////////////////
    ...
    msleep(10);
  //  printk("swf55 interrupt  register start2222  \n");
	mt_eint_set_sens(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_SENSITIVE);
	mt_eint_set_hw_debounce(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_CN);


	   // printk("swf55 interrupt  register start33333  \n");
	mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_POLARITY, _DrvPlatformLyrFingerTouchInterruptHandler, 1); 
	msleep(50);
	   // printk("swf55 interrupt  register start4444  \n");
	mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
	msleep(200);
	////////////////////////
	   ...
	   // printk("swf55 interrupt  register INIT_WORK  \n");
        INIT_WORK(&_gFingerTouchWork, _DrvPlatformLyrFingerTouchDoWork);
#else
