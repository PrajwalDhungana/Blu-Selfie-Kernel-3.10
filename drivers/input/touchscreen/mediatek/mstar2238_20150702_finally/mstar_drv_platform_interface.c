...
s32 /*__devinit*/ MsDrvInterfaceTouchDeviceProbe(struct i2c_client *pClient, const struct i2c_device_id *pDeviceId)
{
   ...
   // DrvPlatformLyrTouchDeviceRequestGPIO();  //only for sprd and qcom //swf delete
    ...
    DrvPlatformLyrTouchDevicePowerOn();
    nRetVal = DrvMainTouchDeviceInitialize();//swf delete nRetVal=-19
   //   printk("swf55 stag44a line=%d,nRetVal=%d \n",__LINE__,nRetVal); 
    if (nRetVal == -ENODEV)
    {
     ...
     // printk("swf55 stag55 line=%d\n",__LINE__ );
    DrvPlatformLyrTouchDeviceRegisterFingerTouchInterruptHandler();

     // printk("swf55 stag66 line=%d\n",__LINE__ );

   // DrvPlatformLyrTouchDeviceRegisterEarlySuspend(); //only for sprd and qcom //swf delete
  //  printk("swf55bbb line=%d\n",__LINE__ );
...
DrvFwCtrlGetCustomerFirmwareVersion(&nMajor, &nMinor, &pVersion); //fang
  ...
  //  printk("swf55bbb line=%d\n",__LINE__ );
    DrvIcFwLyrCheckFirmwareUpdateBySwId();  //fang  go to "msg2xxx_xxxx_update_bin" finally
