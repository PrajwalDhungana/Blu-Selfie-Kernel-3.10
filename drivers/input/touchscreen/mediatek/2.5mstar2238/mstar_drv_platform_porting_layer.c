...
void DrvPlatformLyrTouchDeviceRegulatorPowerOn(void)
{
    ...
    if (nRetVal)
    {
        printk("msg2238  Could not set to 2800mv.\n");
    }
...
{
    printk("msg2238***DrvPlatformLyrTouchDevicePowerOn***MS_TS_MSG_IC_GPIO_RST=0x%x\n",MS_TS_MSG_IC_GPIO_RST);
...
{
    DBG("*** %s() ***\n", __func__);
    printk("mstar2238  point touch pressed\n");
    ...
    printk("mstar2238  point touch pressed nX=%d,nY=%d\n",nX,nY);
...
{
    DBG("*** %s() ***\n", __func__);
    printk("msg2238 point touch released\n");
...
s32 DrvPlatformLyrInputDeviceInitialize(struct i2c_client *pClient)
{
    ...
    {
        printk("msg2238  input device allocation failed ***\n");
        return -ENOMEM;
    }
...
*/
    ...
    if (nRetVal < 0)
    {
        printk("msg2238  *** Unable to register touch input device ***\n");
    }
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
...
s32 DrvPlatformLyrTouchDeviceRequestGPIO(void)
{
    ...
    if (nRetVal < 0)
    {
        printk("msg2238  Failed to request GPIO %d, error %d ***\n", MS_TS_MSG_IC_GPIO_RST, nRetVal);
    }
...
s32 DrvPlatformLyrTouchDeviceRegisterFingerTouchInterruptHandler(void)
{
    ...
    printk("msg2238***CUST_EINT_TOUCH_PANEL_NUM=0x%x ***\n",CUST_EINT_TOUCH_PANEL_NUM);
    ...
    {    	
        ...
        if (nRetVal != 0)
        {
          printk("msg2238**** Unable to claim irq %d; error %d ***\n", MS_TS_MSG_IC_GPIO_INT, nRetVal);
        }
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
        ...
        { 
            ...
            printk("msg2238*Failed to create kernel thread: %d\n", nRetVal);
        }
#endif //CONFIG_USE_IRQ_INTERRUPT_FOR_MTK_PLATFORM
#endif
    }
				   ...
				   void* pBuffOut, int nSizeOut, int* pActualOut)
{
    ...
    switch (nCommand)
    {
        ...
        case SENSOR_ENABLE:
            ...
            else
            {
                ...
                if (nValue)
                {
                    ...
                    {
                        printk("msg2238  Enable ps fail: %d\n", nErr);
                        return -1;
                    }
                }
                else
                {
                    ...
                    {
                        printk("msg2238  Disable ps fail: %d\n", nErr);
                        return -1;
                    }
                }
            }
        ...
        case SENSOR_GET_DATA:
            ...
            {
                printk("msg2238  Get sensor data parameter error!\n");
                nErr = -EINVAL;
            }
            else
            {
       ...
       default:
           printk("msg2238  Un-recognized parameter %d!\n", nCommand);
           nErr = -1;
           break;
    }
