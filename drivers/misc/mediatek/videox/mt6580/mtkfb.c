...
//2014-9-4 humingming add for lcm info
#define LCT_MTK_LCM_INFO_SUPPORT  

#ifdef LCT_MTK_LCM_INFO_SUPPORT
#include <linux/proc_fs.h>
#include <linux/fs.h>
#define LCM_PROC_FILE "lcm_info"
static struct proc_dir_entry *g_lcm_proc = NULL;
#endif
...
//2014-9-4 humingming add for lcm info
#ifdef LCT_MTK_LCM_INFO_SUPPORT
static int lcm_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	if(mtkfb_find_lcm_driver_info())
	{
		...
		cnt = sprintf(page, "NULL\n");
	*eof = 1;
	printk("Leave lcm_proc_read. cnt = %d\n", cnt);
	return cnt;
}
#endif
...
//lj
#ifdef LCT_MTK_LCM_INFO_SUPPORT
static int lcm_info_proc_show(struct seq_file *m, void *unused)
{
      seq_printf(m, "LCM::%s\n",mtkfb_lcm_name);
	return 0;
}
...
{
	return single_open(file, lcm_info_proc_show, NULL);
}
...
//2014-9-4 humingming add for lcm info  lj
#ifdef LCT_MTK_LCM_INFO_SUPPORT
/*
	g_lcm_proc = create_proc_entry(LCM_PROC_FILE, 0444, NULL);
	if (g_lcm_proc == NULL) {
		printk("create_proc_entry failed\n");
	} else {
		g_lcm_proc->read_proc = lcm_proc_read;
		g_lcm_proc->write_proc = NULL;
		printk("create_proc_entry success\n");
	}
	*/
	//lj  ctp_vendor_proc_fops
	if(proc_create(LCM_PROC_FILE, 0444, NULL, &lcm_info_proc_fops)== NULL)   
	{
		printk("lj create_proc_entry lcm lcm_info_proc_fops  failed\n");
	}
