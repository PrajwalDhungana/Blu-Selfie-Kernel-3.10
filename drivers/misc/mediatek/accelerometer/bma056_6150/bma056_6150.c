...
#include <accel.h>
#endif
#define USE_NEW_SENSOR_ARCH ///lc mike_zhu 20150605
#include <accel.h>
#include <cust_acc.h>
#include <linux/hwmsensor.h>
#include <linux/hwmsen_dev.h>
#include <linux/sensors_io.h>
#include "bma056_6150.h"
...
/*----------------------------------------------------------------------------*/
#define GSE_TAG                  "[Gsensor] bma056_6150 "
			...
			u8 addr, u8 *data, u8 len)
{
	...
	if (err != 2) {
		printk(" \n");
		GSE_ERR("--line354--i2c_transfer error: (%d %p %d) %d\n",addr, data, len, err);
		err = -EIO;
	} else {
		err = 0;/*no error*/
	}
...
static int BMA255_CheckDeviceID(struct i2c_client *client)
{
	...
	printk("BMA255_CheckDeviceID ------enter-----\n ");
	memset(databuf, 0, sizeof(u8)*2);    

	res = bma_i2c_read_block(client, BMA255_REG_DEVID, databuf, 0x01);
	printk("BMA255_CheckDeviceID ------0----%d  \n ", databuf[0]);
	//res = bma_i2c_read_block(client, BMA255_REG_DEVID, databuf, 0x01);
	if(res < 0)
		goto exit_BMA255_CheckDeviceID;
	printk("BMA255_CheckDeviceID ------ok----%d  \n ", databuf[0]);
...
//lc mike_zhu 20150605
	...
	printk("bma056_6150.c bma255_i2c_probe \n");
...
#if 1//lc mike_zhu 20150605
	for(retry = 0; retry < 3; retry++){
		if(0 != (err = bma255_init_client(new_client, 1)))
		{
			printk("bma255_device init cilent fail time: %d\n", retry);
			continue;
		}
	}
	if(err != 0)
...
#endif
	printk("bma056_6150.c bma255_i2c_probe------1------- \n");
	//allocate DMA buffer
	I2CDMABuf_va = (u8 *)dma_alloc_coherent(NULL, DMA_BUFFER_SIZE, &I2CDMABuf_pa, GFP_KERNEL);
	if(I2CDMABuf_va == NULL)
	{
		err = -ENOMEM;
		printk("bma056_6150.c Allocate DMA I2C Buffer failed! error = %d\n", err);
		goto exit_dma_alloc_failed;
		
	}
	printk("bma056_6150.c bma255_i2c_probe------2------- \n");
	if(err = misc_register(&bma255_device))
	{
		printk("bma056_6150.c register failed\n");
...
#endif
	{
		printk("bma056_6150 create attribute err = %d\n", err);
	...
	printk("bma056_6150.c bma255_i2c_probe------3------- \n");
...
static int __init bma255_init(void)
{
	...
	GSE_FUN();
	printk("bma056_6150.c bma255_init =hw->i2c_num=%d",hw->i2c_num);
	...
	{
		printk("bma056_6150 failed to register driver");
		return -ENODEV;
	}
#endif
	return 0;    
