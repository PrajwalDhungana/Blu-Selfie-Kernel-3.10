...
/* log macro */
#define GSE_TAG                  "[gsensor] bma2xx.c"
				...
				u8 *data, u8 len)
{
	...
	if (!client)
		...
		printk("bma2xx  length %d exceeds %d\n", len, C_I2C_FIFO_SIZE);
		return -EINVAL;
	}

	err = i2c_transfer(client->adapter, msgs, sizeof(msgs)/sizeof(msgs[0]));
	if (err != 2) {
		printk("bma2xx i2c_transfer error: (%d %p %d) %d\n",
			addr, data, len, err);
		err = -EIO;
	} else {
		err = 0;/*no error*/
				...
				u8 *data, u8 len)
{
	...
	*/
	...
	if (err < 0) {
		printk("bma2xx send command error!!\n");
		return -EFAULT;
	} else {
		err = 0;/*no error*/
	}
...
#if 0///vio18 vio28   lc mike_zhu 20150513
...
static int bma_read_raw_data(struct i2c_client *client, s16 data[BMA_AXES_NUM])
{
	...
	if (priv->power_mode == BMA_SUSPEND_MODE) {
		...
		if (err < 0) {
			printk("bma2xx set power mode failed, err = %d\n", err);
			return err;
		}
	}
	...
	if (priv->sensor_type == BMA220_TYPE) {/* BMA220 */
		...
		if (err) {
			printk("bma2xx [%s]read raw data failed, err = %d\n",
		...
		unsigned long arg)
{
	...
	if (err) {
		printk("bma2xx access error: %08x, (%2d, %2d)\n",
...
static int bma_suspend(struct i2c_client *client, pm_message_t msg)
{
	...
	if (msg.event == PM_EVENT_SUSPEND) {
		...
		///vio18 vio28   lc mike_zhu 20150513 bma_power(obj->hw, 0);
	}
	return err;
}
...
static int bma_resume(struct i2c_client *client)
{
	...
	///vio18 vio28   lc mike_zhu 20150513 bma_power(obj->hw, 1);
		...
		const struct i2c_device_id *id)
{
	...
	if (err) {
		printk("bma2xx invalid direction: %d\n", obj->hw->direction);
	...
	if (err) {
		printk("bma2xx misc device register failed, err = %d\n", err);
		goto exit_misc_device_register_failed;
	}

	err = bma_create_attr(&bma_gsensor_driver.driver);
	if (err) {
		printk("bma2xx create attribute failed, err = %d\n", err);
...
static int bma_probe(struct platform_device *pdev)
{
	...
	///vio18 vio28   lc mike_zhu 20150513 bma_power(hw, 1);
	if (i2c_add_driver(&bma_i2c_driver)) {
		printk("bma2xx add i2c driver failed\n");
		return -1;
	}
...
static int bma_remove(struct platform_device *pdev)
{
	...
	///vio18 vio28   lc mike_zhu 20150513 bma_power(hw, 0);
...
static int __init bma_init(void)
{
	...
	printk("bma2xx :hw->i2c_num:=%d bosch accelerometer driver version: %s\n",hw->i2c_num, BMA_DRIVER_VERSION);

	i2c_register_board_info(hw->i2c_num, &bma_i2c_info, 1);

	if (platform_driver_register(&bma_gsensor_driver)) {
		printk("bma2xx register gsensor platform driver failed\n");
		return -ENODEV;
	}
	return 0;
}
