...
static int s4AF_ReadReg(unsigned short *a_pu2Result)
{
    ...
    if (i4RetValue < 0)
    {
        printk("[DW9714AF] I2C read failed!! \n");
        return -1;
    }
...
static int s4AF_WriteReg(u16 a_u2Data)
{
    ...
    //printk("[DW9714AF] g_sr %d, write %d \n", g_sr, a_u2Data);
    //g_pstAF_I2Cclient->ext_flag |= I2C_A_FILTER_MSG;
    i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);

    if (i4RetValue < 0)
    {
        printk("[DW9714AF] s4AF_WriteReg I2C send failed!! \n");
        return -1;
    }
...
inline static int getAFInfo(__user stDW9714AF_MotorInfo * pstMotorInfo)
{
    ...
    {
        printk("[DW9714AF] copy to user failed when getting motor information \n");
    }
...
{
    int ret = 0;
        printk("[DW9714AF] moveAF \n");

    if((a_u4Position > g_u4AF_MACRO) || (a_u4Position < g_u4AF_INF))
    {
        printk("[DW9714AF] out of range \n");
        return -EINVAL;
    }
    ...
    //printk("[DW9714AF] move [curr] %d [target] %d\n", g_u4CurrPosition, g_u4TargetPosition);
...
{
    printk("[DW9714AF] AF_Open Start \n");


    if(g_s4AF_Opened)
    {
        printk("[DW9714AF] The device is opened \n");
        return -EBUSY;
    }
...
{
    printk("[DW9714AF] AF_Release Start \n");

    if (g_s4AF_Opened)
    {
        printk("[DW9714AF] Free \n");
        g_sr = 5;
        s4AF_WriteReg(200);
        msleep(10);
        s4AF_WriteReg(100);
...
inline static int Register_AF_CharDrv(void)
{
    ...
    printk("[DW9714AF] Register_AF_CharDrv Start\n");

    //Allocate char driver no.
    if( alloc_chrdev_region(&g_AF_devno, 0, 1,AF_DRVNAME) )
    {
        printk("[DW9714AF] Allocate device no failed\n");
    ...
    if(NULL == g_pAF_CharDrv)
    {
        ...
        printk("[DW9714AF] Allocate mem for kobject failed\n");
    ...
    {
        printk("[DW9714AF] Attatch file operation failed\n");
    ...
    if (IS_ERR(actuator_class)) {
        ...
        printk("[DW9714AF] Unable to create class, err = %d\n", ret);
        return ret;
    }
...
{
    printk("[DW9714AF] Unregister_AF_CharDrv Start\n");
...
static int AF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk("[DW9714AF] AF_i2c_probeStart\n");
    ...
    if(i4RetValue){
        ...
        printk("[DW9714AF]  register char device failed!\n");
...
static int __init DW9714AF_i2C_init(void)
{
    ...
    if(platform_device_register(&g_stAF_device)){
        printk("DW9714AF_i2C_init failed to register AF driver\n");
        return -ENODEV;
    }
