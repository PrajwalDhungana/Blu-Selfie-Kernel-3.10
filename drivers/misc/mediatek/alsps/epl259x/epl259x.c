...
#define MT6580M
#if defined(MT6575) || defined(MT6571) || defined(MT6589) || defined(MT6580M) || defined(MT6592) || defined(MT6735) || defined(MT6752) || defined(MT6753)
#include <mach/devs.h>
...
#if defined(MT6580M) || defined(MT6592) || defined(MT6735) || defined(MT6752) || defined(MT6753)
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    epl_sensor.ps.gain = EPL_GAIN_MID;  //lj EPL_GAIN_LOW
#if PS_DYN_K
...
static int epl_sensor_init_client(struct i2c_client *client)
{
    ...
    {
#if defined(MT6580M) || defined(MT6592) || defined(MT6752) || defined(MT6735) || defined(MT6753)
        mt_eint_mask(CUST_EINT_ALS_NUM);
#else
    //    mt65xx_eint_mask(CUST_EINT_ALS_NUM);
#endif
...
#if PS_DYN_K
#if PS_DYN_K_STR
        len += snprintf(buf+len, PAGE_SIZE-len, "Dyn enhance ch0 = %hd \n", dynk_enhance_ch0);
#endif
...
/*----------------------------------for factory mode test---------------------------------------*/
		...
		case ALSPS_SET_PS_THRESHOLD:
			...
			break;
#endif //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
//hzk add for midtest psensor cab start 
		case ALSPS_GET_PS_THRESHOLD_VALUE: 
			threshold[0] = dynk_thd_high;
			...
			break;
//hzk add for midtest psensor cab end
...
static int __init epl_sensor_init(void)
{
	...
	//struct i2c_board_info i2c_epl_sensor={ I2C_BOARD_INFO(EPL_DEV_NAME,hw->i2c_addr[0] )};
...
/*----------------------------------------------------------------------------*/
static void epl259x_remove(void)
{
//lj: need do
	return 0;
}
