...
#if 1///lc mike_zhu 20150605  def CONFIG_OF
...
#define MSE_TAG					"[Msensor] bmm056_6150.c "
...
int retry;
	...
	printk("bmm050_checkchipid read chip id result: %#x", chip_id);
for(retry=0;retry<3;retry++)///mike_zhu 20150607
{
hwmsen_read_block(client, BMM050_CHIP_ID, &chip_id, 1);
printk("bmm050_checkchipid read chip id result: %#x retry=%d \n", chip_id,retry);
...
{
	struct mag_hw *hw = get_cust_mag_hw();
printk("bmm056_6150.c bmm050_init =hw->i2c_num=%d",hw->i2c_num);
	...
	{
		printk("bmm056_6150.c failed to register driver");
		return -ENODEV;
	}
#endif
	return 0;    
